          ...
          {TOOLS.map((t, i) => (
            <Reveal key={t.name} delay={Math.min(i * 0.05, 0.2)}>
              <a
                ...
                className="group block border border-border bg-surface p-7 md:p-8 h-full border-l-2 border-l-transparent hover:border-l-accent transition-all duration-300"
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-accent border border-accent/40 px-2 py-1 whitespace-nowrap">
                    ✦ {t.status}
                  </span>
                </div>
